...
int main(int argc, char *argv[]) {
  ...
  // 로그가 저장될 경로 설정 => 환경변수 파일을 입력하여 그 입력된 파일을 분석 후 로그 파일을 log_path에 저장해주는 함수임.
  set_logfile(log_file, log_path);

  // 로그가 저장될 파일 오픈
  if ((file_fd = open(log_path, O_WRONLY | O_CREAT | O_APPEND)) == -1) {
    printf("%s 파일이 존재하지 않습니다.", log_path);
    exit(0);
  }
  ...
  if (fork() == 0) {
    while(1) {
      ...
      if ((clnt_fd = accept(sock_fd, (struct sockaddr*) &clnt_addr, &addr_len)) == -1) {
        ...
        // 클라이언트가 접속되면 "클라이언트 연결 생성"이라는 메세지가 전달되는데, 이 메세지를 읽은 후 msg.message에 저장함
        if (read(clnt_fd, msg.message, MASSAGE_SIZE) == -1) {
          puts("클라이언트가 전달한 접속 연결 메세지 읽기 에러");
          exit(0);
        };
        
        // 클라이언트가 전달한 메세지를 파일(file_fd)에 저장함
        save_log(msg.message, msg, file_fd, argv[1]);
      }
      ...
      if (fork() == 0) {
        ...
        while(1) {
          // 클라이언트의 요청 데이터 읽기
          if (read(clnt_fd, msg.message, MASSAGE_SIZE) > 0) {
            ...
            if (!strncmp(msg.message, "exit", 4)) {
              ...
              read(clnt_fd, msg.message, MASSAGE_SIZE); // "클라이언트 연결 종료" 메시지 읽기
              save_log(msg.message, msg, file_fd, argv[1]); // "클라이언트 연결 종료" 메시지 파일에 저장
